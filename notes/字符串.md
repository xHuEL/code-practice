...
[最后一个单词的长度](https://leetcode.cn/problems/length-of-last-word/) —— [提交记录](../code/length-of-last-word.py)

[二进制求和](https://leetcode.cn/problems/add-binary/) —— [提交记录]()
...
对于下面的题目，应该是简单使用堆栈就可以解决

[有效的括号](https://leetcode.cn/problems/valid-parentheses/) —— [提交记录](../code/valid-parentheses.py)

<b>练习题</b>：

[不同的子序列](https://leetcode.cn/problems/distinct-subsequences/) —— [提交记录](../code/distinct-subsequences.py)
...
3. <b>字符串哈希</b>
   ...
   [字母异位词分组](https://leetcode.cn/problems/group-anagrams/) —— [提交记录](../code/group-anagrams.py)

   到目前碰到两个异位词的题目（碎碎念）。这个题目主要是找到所有相同的异位词，一般有两种思路: 第一种思路，统计每个词每个字符的数量，并使用一个dict（字典）来存储，然后两个for循环进行比较，伪代码如下：
   ...
   第二种思路就是使用hash来表示一个异位词，然后比较每个词的hash值，相同hash值则认为是异位词，从思想来说其实很简单，在异位词的hash词实现上，其实是取巧的
   ...
   对于一个数组的hash值求解，可以使用tuple来对数组进行包装
...
5. <b>递归实现</b>
   ...
   瞟了一眼好多题目，感觉都是使用递归来实现的。使用递归来实现主要是有些剪枝条件要注意，还是就是构造递归函数，在写递归过程可能一开始不太适应，跟动态规划

## 两个字符

1.  <b>字符串模式匹配</b>（说人话：就是判断一个字符串是否在另外字符串中出现。或者找到在另外一个字符串出现的位置）

   这个类型的题目有好几个经典的算法，适合不同的变种。

   (1) <b>hash实现</b>

   [重复的DNA序列](https://leetcode.cn/problems/repeated-dna-sequences/) —— [提交记录](../code/repeated-dna-sequences.py)

   多项式hash（下面是gpt给出的多项式hash的定义)  

   ![image-20260108074126473](../notes/pic/多项式hash.png)
